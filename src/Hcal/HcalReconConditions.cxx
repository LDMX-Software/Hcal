...
double HcalReconConditions::linearize(const ldmx::HcalDigiID& id,
                                      double sum_tot) const {
...
class HcalReconConditionsProvider : public framework::ConditionsObjectProvider {
 ...
 public:
   ...
   */
  HcalReconConditionsProvider(const std::string& name,
    ...
    if (name != HcalReconConditions::CONDITIONS_NAME) {
      EXCEPTION_RAISE("BadConfig",
                      "The name provided to HcalReconConditionsProvider " +
   ...
   *
   * @note Expects the parent condition tables to all be
  ...
  getCondition(const ldmx::EventHeader& context) final override {
    ...
    auto [adc_gain_co, adc_gain_iov] =
        requestParentCondition(adc_gain_, context);
