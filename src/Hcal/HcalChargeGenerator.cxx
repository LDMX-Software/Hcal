...
//g++ HcalChargeGenerator.cxx -std=c++11 -I../../ -I$CLHEP_INCLUDE_DIR -L$CLHEP_LIB_DIR -lCLHEP -DSiPMChargesStandalone `root-config --cflags --glibs`

namespace hcal
{

double HcalChargeGenerator::GetAvalancheProbability(double v)
{
...
std::vector<std::pair<int,int> > HcalChargeGenerator::FindCrossTalkPixelIds(const std::pair<int,int> &pixelId)
{
...
std::pair<int,int> HcalChargeGenerator::FindThermalNoisePixelId()
{
...
std::pair<int,int> HcalChargeGenerator::FindFiberPhotonsPixelId()
{
  double x,y;
...
bool HcalChargeGenerator::IsInactivePixelId(const std::pair<int,int> &pixelId)
{
...
double HcalChargeGenerator::GenerateAvalanche(Pixel &pixel, const std::pair<int,int> &pixelId, double time, size_t photonIndex, bool darkNoise)
{
...
double HcalChargeGenerator::GetVoltage(const Pixel &pixel, double time)
{
...
void HcalChargeGenerator::SetSiPMConstants(int nPixelsX, int nPixelsY, double overvoltage, double timeConstant,
...
void HcalChargeGenerator::FillQueue(const std::vector<std::pair<double,size_t> > &photons, double startTime, double endTime)
{
...
void HcalChargeGenerator::Simulate(const std::vector<std::pair<double,size_t> > &photons,   //pair of photon time and index in the original photon vector
...
HcalChargeGenerator::HcalChargeGenerator(CLHEP::RandFlat &randFlat, CLHEP::RandPoissonQ &randPoissonQ, const std::string &photonMapFileName) :
...
int main()
{
  ...
  std::vector<hcal::SiPMresponse> SiPMresponseVector;

  hcal::HcalChargeGenerator::ProbabilitiesStruct probabilities;
  ...
  hcal::HcalChargeGenerator sim(randFlat,randPoissonQ,"/cvmfs/mu2e.opensciencegrid.org/DataFiles/CRVConditions/v6_0/photonMap.root");
