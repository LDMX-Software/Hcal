...
void HcalPhotonGenerator::LoadLookupTable(const std::string &filename, int debug)
{
  _fileName = filename;
  ...
  if(_LC.reflector!=0 && _LC.reflector!=1 && _LC.reflector!=2) 
                      throw std::logic_error("Lookup tables can have either no reflector, or a reflector on the +z side.");
...
HcalPhotonGenerator::~HcalPhotonGenerator()
{
}

void HcalPhotonGenerator::MakePhotons(const CLHEP::Hep3Vector &stepStartTmp,   //they need to be points
...
bool HcalPhotonGenerator::IsInsideScintillator(const CLHEP::Hep3Vector &p)
{
...
bool HcalPhotonGenerator::IsInsideFiber(const CLHEP::Hep3Vector &p, const CLHEP::Hep3Vector &dir, double &r, double &phi)
{
  if(fabs(p.z())>=_LC.halfLength) return false;
...
double HcalPhotonGenerator::GetRandomTime(const LookupBin *theBin)
{
...
int HcalPhotonGenerator::GetRandomFiberEmissions(const LookupBin *theBin)
{
  //The lookup tables encodes probabilities as probability*mu2eCrv::LookupBin::probabilityScale(255),
  //so that the probabilities can be stored as integers. For example, the probability of 1 is stored as 255.
  //Due to rounding issues, the sum of all entries for this bin may not be 255.
...
int HcalPhotonGenerator::GetNumberOfPhotonsFromAverage(double average, int nSteps)  //from G4Scintillation
{
  int nPhotons;
  if(average>10.0)
  {
...
int HcalPhotonGenerator::GetNumberOfPhotons(int SiPM)
{
  return _arrivalTimes[SiPM].size();
}

const std::vector<double> &HcalPhotonGenerator::GetArrivalTimes(int SiPM)
{
  return _arrivalTimes[SiPM];
}

//average number of cerenkov photons per millimeter
double HcalPhotonGenerator::GetAverageNumberOfCerenkovPhotons(double beta, double charge, std::map<double,double> &photons)
{
  ...
  for(i=photons.begin(); i!=photons.end(); i++)
  {
    if(beta<=i->first)
    {
      ...
      numberPhotons*=fabs(charge/CLHEP::eplus);
  ...
  return photons.rbegin()->second*fabs(charge/CLHEP::eplus); //this shouldn't happen
}
